              ...
              <Link key={project.id} href={`/dashboard/projects/${project.id}`}>
                <Card className="hover:shadow-lg transition-all duration-200 cursor-pointer group h-full">
                  <CardHeader className="pb-3">
                    ...
                    {/* Member Role Badge - PERMISSION-BASED */}
                    <PermissionGuard condition={!canViewAllProjects && !!(project as any).memberRole}>
                      <div className="flex items-center gap-2 mt-2">
                        <Badge variant="secondary" className={getRoleColor((project as any).memberRole)}>
                          <UserCheck className="w-3 h-3 mr-1" />
                          {(project as any).memberRole === 'supervisor' && <Crown className="w-3 h-3 mr-1" />}
                          {(project as any).memberRole?.charAt(0).toUpperCase() + (project as any).memberRole?.slice(1)}
                        </Badge>
                        {(project as any).joinedAt && (
                          <span className="text-xs text-gray-500">
                            Joined {formatDate((project as any).joinedAt)}
                          </span>
                        )}
                      </div>
                    </PermissionGuard>
