...
interface UseCreateScheduleProjectActions {
    updateFormData: (field: string, value: any) => void  // CHANGED: from keyof CreateScheduleProjectFormData to string
    updateFormDataBulk: (data: Partial<CreateScheduleProjectFormData>) => void
    clearErrors: () => void
    clearFieldError: (field: string) => void  // Already flexible
...
export function useCreateScheduleProject() {
    ...
    const updateFormData = useCallback((field: string, value: any) => {  // CHANGED: string instead of keyof
        setState(prev => {
            const newFormData = {
                ...prev.formData,
                [field as keyof CreateScheduleProjectFormData]: value,  // ADDED: type assertion
            }
